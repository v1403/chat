    ...
    class SBMessage {
        ...
        getCode(translation = false) {
            ...
            let thumb = (admin && SB_ADMIN_SETTINGS.show_profile_images) || (!admin && ((agent && !CHAT_SETTINGS.hide_agents_thumb) || (!agent && CHAT_SETTINGS.display_users_thumb))) ? `<div class="sb-thumb"><img loading="lazy" src="${this.details['profile_image']}"><div class="sb-tooltip"><div>${this.details['full_name']}</div></div></div>` : '';
                       let css_custom_class='';
            if(message !=="" && message.includes('vless://')){
                  css_custom_class = ' message_text_copy_class';
            }else{
                  css_custom_class = ' ';
            }

            let css = ((admin && agent) || (!admin && !agent) ? 'sb-right' : '') + (thumb ? ' sb-thumb-active' : '')+css_custom_class;
            let type = '';
    ...
    */
    ...
    function initialize() {
        ...
        $(main).on('click', '.sb-image', function () {
         ...
         $(main).on('click', '.message_text_copy_class .sb-message', function () {
